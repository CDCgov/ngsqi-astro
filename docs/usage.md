...
Sample1,path/to/data/ERR4678563_1.fastq.gz,path/to/data/ERR4678562_2.fastq.gz
Sample2,path/to/data/ERR4678563_1.fastq.gz,path/to/data/ERR4678563_2.fastq.gz
```
| Column    | Description                                                                                                                                                                            |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
The isolate samplesheet when using the local mode should be formatted as follows:

```
sample_id,added_copy_number,file_path,species_name
GCA_018454105.3,1,path/to/data/GCA_018454105.3_PDT001044797.3_genomic.fna,Acinetobacter baumannii
GCA_016490125.3,1,path/to/data/GCA_016490125.3_PDT000725303.3_genomic.fna,Acinetobacter baumannii
```

The isolate samplesheet when using the download mode should be formatted as follows:

```
sample_id,added_copy_number,file_path,species_name
GCA_018454105.3,1,,Acinetobacter baumannii
GCA_016490125.3,1,,Acinetobacter baumannii
```
Note that the added_copy_number column will vary depending on the user's simulation interests. 
...
| `file_path` | Full path to isolate genome file (.fna)                                                             |
| `species_name` | Name of isolate species                                                             |

Two [example samplesheets](../assets/samplesheets/) have been provided with the pipeline.
...
First, go to the [astro releases page](https://github.com/ngsqi-astro/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
