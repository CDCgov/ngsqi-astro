process filter_abundance {
    ...
    #Process the data lines for phylum-level without prefixes and ensure tab delimiters
    grep -E 'p__|clade' "\$input_file" | grep -v 'c__' | sed -E 's/^.*p__//' | awk '{\$1=\$1; print}' OFS='\\t' >> merged_phylum.txt

    #Process the data lines for species-level without prefixes and ensure tab delimiters
