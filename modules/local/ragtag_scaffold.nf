process RAGTAGSCAFFOLD {
    ...
    input:
    tuple val(sample_id), val(added_copy_number), val(file_path), val(species_name), path(isolate_genome)
    tuple val(sample_id), val(added_copy_number), val(file_path), val(species_name), val(ref_accession), path(ref_genome)

    output:
    tuple val(sample_id), val(added_copy_number), path("scaff_${sample_id}"), emit: ragtag_scaff_dirs
